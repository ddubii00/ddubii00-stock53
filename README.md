...
오늘 BREAKOUT 이벤트는 `max(오늘 현재가, 오늘 장중 고가) >= 신규 돌파가`로 판정합니다. 전고점을 단 1호가라도 넘으면 돌파이며, 2%나 5% 이상 더 넘어야 하는 조건은 없습니다. Telegram dedup용 BREAKOUT 이벤트는 장중 돌파 이력을 유지하지만, UI의 `실시간` 표는 현재가 기준으로 움직여 목표가 위면 BREAKOUT, 다시 아래로 내려오면 PREALERT(접근 범위 밖이면 WATCH)로 표시합니다.
...
- 현재가가 20D 목표가 위로 올라가면 PREALERT → BREAKOUT, 다시 아래로 내려가면 BREAKOUT → PREALERT로 이동. 접근 범위보다 더 내려가면 표에서 숨기되 다음 주기에 계속 재확인
